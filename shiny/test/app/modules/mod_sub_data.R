...
mod_sub_data__server <- function(id) {
  moduleServer(id, function(input, output, session) {
      ...
      reactable(
        ...
        columns = list(
          Variable = colDef(
            ...
            minWidth = 220,
            cell = function(value) {
              star <- if (value %in% fieldsMandatory_data)
                span(" *", style = "color:#CC6677; font-weight:600;")
              else NULL

              div(style = list(display = "inline-flex", alignItems = "center"),
                             value, star)
            }
          ),
          Value = colDef(
    ...
    observeEvent(input$submit_final, {
      ...
      write.csv(build_form(), tmp_file, row.names = FALSE)
      ...
      # If you later want to send the CSV by email, call your mail helper here.
      # send_email(body, subject, receiver, tmp_file)
    })
  })
}
